...
/**
 * 精准扶贫人员入院时间在2019.8.1之前的按照旧的比例计算,入院时间在2019.8.1之后的提高5个点
 * 普通居民，结算时间在2019.10.1日之前的按照就的比例计算，结算时间在2019.10.1之后的提高5个点。
 * @author zhqh9016
 *
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = BootApplition.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class Recount_tm4 {
	...
	public void calculateProcess(HiomsTreatmentTm hiomsTreatmentTm, float preLargePay1, float preLargePay2,
			float preLargePay3) throws ParseException {
		...
		if (Constant.PERSION_CATEGORY_JZ.equals(hiomsTreatmentTm.getJiaofeirenyuanleibie())) {
			...
			if (hiomsTreatmentTm.getBenciqiandabingleiji() + hiomsTreatmentTm.getBencidabingleiji() > 5000) {
				...
				Date ruyuanshijian = hiomsTreatmentTm.getRuyuanshijian();
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				Date new_policy_boundary_time = dateFormat.parse("2019-08-01 00:00:00");
				float pay = 0;
				if(ruyuanshijian.compareTo(new_policy_boundary_time) >=0){
					pay = stage11 * 0.65f + stage12 * 0.75f + stage13 * 0.85f;
				}else{
					pay = stage11 * 0.6f + stage12 * 0.7f + stage13 * 0.8f;
				}
			...
			if (hiomsTreatmentTm.getBenciqiandabingleiji() + hiomsTreatmentTm.getBencidabingleiji() > 12000) {
				...
				Date jiesuanshijian = hiomsTreatmentTm.getJiesuanshijian();
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				Date new_policy_boundary_time = dateFormat.parse("2019-09-30 00:00:00");
				float pay = 0;
				if(jiesuanshijian.compareTo(new_policy_boundary_time) >=0){
					pay = stage11 * 0.6f + stage12 * 0.7f + stage13 * 0.8f;
				}else{
					pay = stage11 * 0.55f + stage12 * 0.65f + stage13 * 0.75f;
				}
		...
		if (Constant.PERSION_CATEGORY_JZ.equals(hiomsTreatmentTm.getJiaofeirenyuanleibie())) {
			...
			if (hiomsTreatmentTm.getBenciqiandabingleiji() + hiomsTreatmentTm.getBencidabingleiji2() > 5000) {
				...
				float pay = 0;
				if(ruyuanshijian.compareTo(new_policy_boundary_time) >=0){
					pay = stage21 * 0.65f + stage22 * 0.75f + stage23 * 0.85f;
				}else{
					pay = stage21 * 0.6f + stage22 * 0.7f + stage23 * 0.8f;
				}
			...
			if (hiomsTreatmentTm.getBenciqiandabingleiji() + hiomsTreatmentTm.getBencidabingleiji2() > 12000) {
				...
				if(jiesuanshijian.compareTo(new_policy_boundary_time) >=0){
					...
					pay = stage21 * 0.55f + stage22 * 0.65f + stage23 * 0.75f;
				}
		...
		if (hiomsTreatmentTm.getBenciqiandabingleiji3() + hiomsTreatmentTm.getBencidabingleiji() > 12000) {
			...
			float pay = 0;
			pay = stage31 * 0.55f + stage32 * 0.65f + stage33 * 0.75f;
