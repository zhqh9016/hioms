...
public class CustomFormAuthenticationFilter
  ...
  protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) {
    // 现在前台传递的用户名密码表单默认为username和password
    String loginName = request.getParameter("loginName");
    String password = request.getParameter("password");

    log.info(loginName + "请求登录");

    return new UsernamePasswordToken(loginName, password.toCharArray());
  }
  ...
  protected boolean onLoginFailure(
    ...
    try {
      ...
      if ("IncorrectCredentialsException".equals(message)) {
        out.println("{\"state\":false,\"message\":\"密码错误\"}");
      } else if ("UnknownAccountException".equals(message)) {
        out.println("{\"state\":false,\"message\":\"账号不存在\"}");
      } else {
        out.println("{\"state\":false,\"message\":\"未知错误\"}");
      }
      out.flush();
      out.close();
